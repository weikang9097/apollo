...
public interface CommitRepository extends PagingAndSortingRepository<Commit, Long> {
  ...
  @Modifying
  @Query("update Commit SET isDeleted = true, deletedAt = ROUND(DATE_PART('EPOCH',now())*1000), dataChangeLastModifiedBy = ?4 where appId=?1 and clusterName=?2 and namespaceName = ?3 and isDeleted = false")
